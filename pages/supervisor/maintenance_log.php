...
<head>
    ...
    <title>Maintenance Log — SmartWash | BatStateU ARASOF-Nasugbu</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .action-cleaning { background: rgba(21,101,192,0.15); color: var(--blue-info); }
        .action-refill { background: rgba(46,125,50,0.15); color: var(--green-success); }
        .action-incident { background: rgba(197,0,0,0.15); color: var(--red-deep); }
        .action-system { background: rgba(249,168,37,0.15); color: #b45309; }
    ...
    <main class="main-content">
        <div class="top-bar">
            <div class="page-title">
                ...
                <p>Complete history of all maintenance actions and sensor triggered event, etc</p>
            </div>
        ...
        <div class="filter-bar">
            ...
            <button class="filter-btn" onclick="filterLogs('cleaning')">Cleaning</button>
            <button class="filter-btn" onclick="filterLogs('refill')">Refills</button>
            <button class="filter-btn" onclick="filterLogs('incident')">Incidents</button>
            ...
            <div class="table-container">
                <table id="logsTable">
                    ...
                    <tbody id="logsTableBody">
                        <tr><td colspan="4" class="empty-state">Loading maintenance logs...</td></tr>
                    </tbody>
                </table>
            </div>
...
<div class="demo-badge" onclick="toggleDemoMode()">🧪 DEMO MODE | Maintenance Log Data</div>
...
let allLogs = [
    { id: 1, datetime: '2026-04-28 09:15:00', restroom: '1F-FR - 1st Floor Front', action: 'Soap refilled', action_type: 'refill', performed_by: 'Rey M. Santos', staff_initials: 'RS' },
    { id: 2, datetime: '2026-04-28 08:45:00', restroom: '2F-MR - 2nd Floor Main', action: 'Air freshener triggered', action_type: 'system', performed_by: 'System Auto', staff_initials: 'SA' },
    { id: 3, datetime: '2026-04-28 08:00:00', restroom: '3F-CR - 3rd Floor CR', action: 'Cleaned and sanitized', action_type: 'cleaning', performed_by: 'Maria L. Cruz', staff_initials: 'MC' },
    { id: 4, datetime: '2026-04-27 16:30:00', restroom: 'GLR2 - Ground Right', action: 'Vape incident reported', action_type: 'incident', performed_by: 'Sensor', staff_initials: 'SN' },
    { id: 5, datetime: '2026-04-27 14:20:00', restroom: 'GLR1 - Ground Left', action: 'Soap refilled', action_type: 'refill', performed_by: 'Rey M. Santos', staff_initials: 'RS' },
    { id: 6, datetime: '2026-04-27 11:00:00', restroom: '2F-LR - 2nd Floor Left', action: 'Cleaned and sanitized', action_type: 'cleaning', performed_by: 'John D. Reyes', staff_initials: 'JR' },
    { id: 7, datetime: '2026-04-27 09:30:00', restroom: '1F-RR - 1st Floor Rear', action: 'Smoke incident reported', action_type: 'incident', performed_by: 'Sensor', staff_initials: 'SN' },
    { id: 8, datetime: '2026-04-26 15:45:00', restroom: 'GLR2 - Ground Right', action: 'Cleaned and sanitized', action_type: 'cleaning', performed_by: 'Maria L. Cruz', staff_initials: 'MC' },
    { id: 9, datetime: '2026-04-26 10:20:00', restroom: '3F-NR - 3rd Floor North', action: 'Soap refilled', action_type: 'refill', performed_by: 'Rey M. Santos', staff_initials: 'RS' },
    { id: 10, datetime: '2026-04-26 08:00:00', restroom: '1F-FR - 1st Floor Front', action: 'Air freshener triggered', action_type: 'system', performed_by: 'System Auto', staff_initials: 'SA' },
    { id: 11, datetime: '2026-04-25 13:30:00', restroom: '2F-MR - 2nd Floor Main', action: 'Cleaned and sanitized', action_type: 'cleaning', performed_by: 'John D. Reyes', staff_initials: 'JR' },
    { id: 12, datetime: '2026-04-25 10:15:00', restroom: 'GLR1 - Ground Left', action: 'Vape incident reported', action_type: 'incident', performed_by: 'Sensor', staff_initials: 'SN' },
    { id: 13, datetime: '2026-04-25 07:45:00', restroom: '3F-CR - 3rd Floor CR', action: 'Soap refilled', action_type: 'refill', performed_by: 'Maria L. Cruz', staff_initials: 'MC' },
    { id: 14, datetime: '2026-04-24 16:20:00', restroom: '1F-RR - 1st Floor Rear', action: 'Cleaned and sanitized', action_type: 'cleaning', performed_by: 'Rey M. Santos', staff_initials: 'RS' },
    { id: 15, datetime: '2026-04-24 09:00:00', restroom: '2F-LR - 2nd Floor Left', action: 'System maintenance', action_type: 'system', performed_by: 'Admin', staff_initials: 'AD' }
];
...
function getActionClass(action_type) {
    ...
    return classes[action_type] || 'action-system';
}
...
function filterLogsByType() {
    ...
    return allLogs.filter(log => log.action_type === currentFilter);
}

function filterLogsByDateRange(logs) {
...
function displayLogs() {
    let filteredLogs = filterLogsByType();
    filteredLogs = filterLogsByDateRange(filteredLogs);
    ...
    if (paginatedLogs.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="empty-state">No maintenance logs found for the selected filters.</td></tr>';
    ...
    tbody.innerHTML = paginatedLogs.map(log => `
        <tr>
            <td style="white-space: nowrap;">${formatDateTime(log.datetime)}</td>
            <td>${escapeHtml(log.restroom)}</td>
            <td>
                <span class="action-badge ${getActionClass(log.action_type)}">
                    ${getActionIcon(log.action_type)} ${escapeHtml(log.action)}
                </span>
            </td>
            <td>
                <div class="staff-name">
                    <div class="staff-avatar">${escapeHtml(log.staff_initials)}</div>
...
function filterLogs(filter) {
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
        ...
        if (btn.textContent.toLowerCase().includes(filter.toLowerCase()) || (filter === 'all' && btn.textContent === 'All Logs')) {
...
function exportLogs() {
    ...
    // Create CSV content
    const headers = ['Date & Time', 'Restroom', 'Action', 'Performed By'];
    ...
    a.href = url;
    a.download = `maintenance_log_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    if (USE_DEMO_MODE) {
        alert(`[DEMO] CSV export would download ${filteredLogs.length} log entries.`);
    }
}
...
function toggleDemoMode() {
    if (confirm('Toggle between Demo and Live mode?\n\nCurrently in: DEMO MODE\n\nNote: Live mode would connect to:\n- api/get_logs.php\n- Real maintenance logs from database')) {
        alert('Switching to live mode would fetch real maintenance logs from the database.');
    }
}
